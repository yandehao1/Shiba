    ...
    public class SpecimenRt_handler : IHttpHandler
    {
        ...
        private static void SaveData(HttpContext context)
        {
            ...
            try
            {
                ...
                if (mode == "ins")
                {
                    ...
                    model_SpecimenRt._id = bll_SpecimenRt.GetMax_id();//主键赋值
                }
        ...
        private static void QueryData(HttpContext context, bool export)
        {
            ...
            if (export)
            {
                ...
                pageAction.Fields = "id as 序号自增列,patientid as 患者唯一标识号,patientname as 患者名称,visitid as 患者就诊号,sampleid as 样本Id,samplename as 样本Name,otherinfo as 其他信息（样本类型、总管数、在库管数、出库管数、创建时间、更新时间）";
                DbHelper.GetTable(pageAction.Sql, ref export_dataTable);
                commExcel._ExportExcel(export_dataTable, "主要作用是用于判断是否需要回发给His");
                result rlt = new result();
                rlt.success = true;
                rlt.msg = commExcel._Url("主要作用是用于判断是否需要回发给His");
                context.Response.Write(JSONHelper.Convert2Json(rlt));
            }
            else
            {
        ...
        private static void ChartData(HttpContext context)
        {
            ...
            if (context.Request["swf"] != null)
                swf = context.Request["swf"];
            string strSql = @"select 字段名,count(*) records from specimenrt group by 字段名";
            DataTable dt = new DataTable();

    ...
    public class LabTestMaster_handler : IHttpHandler
    {
        ...
        private static void SaveData(HttpContext context)
        {
            ...
            try
            {
                ...
                if (mode == "ins")
                {
                    ...
                    model_LabTestMaster._id = bll_LabTestMaster.GetMax_id();//主键赋值
                }
        ...
        private static void QueryData(HttpContext context, bool export)
        {
            ...
            if (export)
            {
                ...
                pageAction.Fields = "id as id,testno as 检验申请单号,priorityindicator as 优先标志,workingid as 工作单号,testcause as 检验目的,relevantclinicdiag as 临床诊断,specimen as 标本,spcmreceiveddatetime as 采样时间,orderingdept as 开医嘱科室,orderingprovider as 医生工号,performedby as 执行科室,resultstatus as 执行情况,resultsrptdatetime as 报告完成时间,transcriptionist as 报告者工号,verifiedby as 审核者工号";
        ...
        private static void ChartData(HttpContext context)
        {
            ...
            if (context.Request["swf"] != null)
                swf = context.Request["swf"];
            string strSql = @"select 字段名,count(*) records from labtestmaster group by 字段名";
            DataTable dt = new DataTable();

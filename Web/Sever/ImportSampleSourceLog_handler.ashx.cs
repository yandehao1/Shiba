    ...
    public class ImportSampleSourceLog_handler : IHttpHandler
    {
        ...
        private static void SaveData(HttpContext context)
        {
            ...
            try
            {
                ...
                if (mode == "ins")
                {
                    ...
                    model_ImportSampleSourceLog._id = bll_ImportSampleSourceLog.GetMax_id();//主键赋值
                }
        ...
        private static void QueryData(HttpContext context, bool export)
        {
            ...
            if (export)
            {
                ...
                pageAction.Fields = "id as 自增列,samplesourcename as 样本源Name,samplesourcetype as 样本源类型,samplesourcedescription as 样本源描述,patientid as 患者唯一标识,patientname as 患者姓名,patientsex as 患者性别,importstatus as 导入数据之前的状态,hidden as 隐藏域数据,resultstatus as 导入数据之后的状态,importdate as 执行导入的时间";
                DbHelper.GetTable(pageAction.Sql, ref export_dataTable);
                commExcel._ExportExcel(export_dataTable, "FP中样本源记录");
                result rlt = new result();
                rlt.success = true;
                rlt.msg = commExcel._Url("FP中样本源记录");
                context.Response.Write(JSONHelper.Convert2Json(rlt));
            }
            else
            {
        ...
        private static void ChartData(HttpContext context)
        {
            ...
            if (context.Request["swf"] != null)
                swf = context.Request["swf"];
            string strSql = @"select 字段名,count(*) records from importsamplesourcelog group by 字段名";
            DataTable dt = new DataTable();

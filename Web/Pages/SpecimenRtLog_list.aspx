...
<table id="datagrid" title="回发数据记录存档" class="easyui-datagrid" style="width:auto;height:460px"
    ...
    <thead>    
			<tr>
                ...
                <th field="username" width="100" sortable="true">回发此数据的用户</th>
                <th field="patiendid" width="100" sortable="true">回发的患者唯一标识</th>
                <th field="sampleid" width="100" sortable="true">回发的样本id</th>
                <th field="postbackstatus" width="100" sortable="true">回发后的状态</th>
...
<script type="text/javascript">
	...
	function newForm(){
		$('#dlg').dialog({    
            ...
            closed: false,  
            cache: false,    
            href: 'SpecimenRtLog_info.aspx?mode=ins'
        });     
	}
	    ...
	    if(rows.length>0){
	       if(rows.length==1){
				...
				$('#dlg').dialog({    
                    ...
                    closed: false,    
                    cache: true,    
                    href: 'SpecimenRtLog_info.aspx?mode=inf&pk='+ row.id
                });     
			}else{ 
	    ...
	    if(rows.length>0){
	       if(rows.length==1){
				...
				$('#dlg').dialog({    
                    ...
                    closed: false,    
                    cache: true,    
                    href: 'SpecimenRtLog_info.aspx?mode=upd&pk='+ row.id
                });     
			}else{ 
	...
	function destroy(){
		...
		if(rows.length>0){ 
				...
				if (r){
						$.post('SpecimenRtLog_handler.ashx?mode=del&pk='+pkSelect,function(result){
							if (result.success){

...
<body>
<!--datagrid栏--> 
<table id="datagrid" title="LabTestResult" class="easyui-datagrid" style="width:auto;height:460px"
             url="LabTestResult_handler.ashx?mode=qry" fit='false'
    ...
    <thead>    
			<tr>
                ...
                <th field="abnormalindicator" width="100" sortable="true">N-正常L-低H-高</th>
                <th field="result" width="100" sortable="true">正文描述</th>
                <th field="units" width="100" sortable="true">检验结果为数值型</th>
...
<script type="text/javascript">
	...
	function newForm(){
		$('#dlg').dialog({    
            title: 'LabTestResult-添加数据',    
            width: 650, 
            height: 450,    
            closed: false,  
            cache: false,    
            href: 'LabTestResult_info.aspx?mode=ins'
        });     
	}
	...
	function infoForm(){
	    ...
	    if(rows.length>0){
	       if(rows.length==1){
				...
				$('#dlg').dialog({    
                    title: 'LabTestResult-查看数据',    
                    width: 650,    
                    height: 450,    
                    closed: false,    
                    cache: true,    
                    href: 'LabTestResult_info.aspx?mode=inf&pk='+ row.id
                });     
			}else{ 
	...
	function editForm(){
	    ...
	    if(rows.length>0){
	       if(rows.length==1){
				var row = $('#datagrid').datagrid('getSelected');
				$('#dlg').dialog({    
                    title: 'LabTestResult-修改数据',    
                    width: 650,    
                    height: 450,    
                    closed: false,    
                    cache: true,    
                    href: 'LabTestResult_info.aspx?mode=upd&pk='+ row.id
                });     
			}else{ 
	...
	function destroy(){
		...
		if(rows.length>0){ 
				...
				if (r){
						$.post('LabTestResult_handler.ashx?mode=del&pk='+pkSelect,function(result){
							if (result.success){
	...
	function searchData(){
		...
		var QryUrl='LabTestResult_handler.ashx?mode=qry&'+Parm; 

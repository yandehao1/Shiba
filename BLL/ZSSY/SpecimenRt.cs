...
namespace RuRo.BLL.ZSSY
{
	...
	{
		private readonly RuRo.DAL.ZSSY.SpecimenRt dal=new RuRo.DAL.ZSSY.SpecimenRt();
		public SpecimenRt()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public RuRo.Model.ZSSY.SpecimenRt GetModelByCache(int Id)
		{
			
			string CacheKey = "SpecimenRtModel-" + Id;
			...
			return (RuRo.Model.ZSSY.SpecimenRt)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<RuRo.Model.ZSSY.SpecimenRt> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<RuRo.Model.ZSSY.SpecimenRt> DataTableToList(DataTable dt)
		{
			List<RuRo.Model.ZSSY.SpecimenRt> modelList = new List<RuRo.Model.ZSSY.SpecimenRt>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				RuRo.Model.ZSSY.SpecimenRt model;

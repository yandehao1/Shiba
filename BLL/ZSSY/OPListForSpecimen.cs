...
namespace RuRo.BLL.ZSSY
{
	...
	{
		private readonly RuRo.DAL.ZSSY.OPListForSpecimen dal=new RuRo.DAL.ZSSY.OPListForSpecimen();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public RuRo.Model.ZSSY.OPListForSpecimen GetModelByCache(int Id)
		{
			
			string CacheKey = "OPListForSpecimenModel-" + Id;
			...
			return (RuRo.Model.ZSSY.OPListForSpecimen)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<RuRo.Model.ZSSY.OPListForSpecimen> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<RuRo.Model.ZSSY.OPListForSpecimen> DataTableToList(DataTable dt)
		{
			List<RuRo.Model.ZSSY.OPListForSpecimen> modelList = new List<RuRo.Model.ZSSY.OPListForSpecimen>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				RuRo.Model.ZSSY.OPListForSpecimen model;
